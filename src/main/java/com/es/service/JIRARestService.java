...
//	            assignScreens = assignScreensToCustomFields();
	            ...
	            for (int i = 0; i < issues.size(); i++) {
	                ...
	                int originalEstimate = (originalEstimateElement != null && !originalEstimateElement.isJsonNull())
	                ...
	                ImportTask taskInfo = new ImportTask(SprintId, issueName, issueId, issueDescription, 5, aiEstimate, 0,
	                        labelsList, 3, taskEstimates, storyPoints, originalEstimate, priority);
...
return null ;
	...
	public void updateToJIRA(String taskId, int newAiestimates) {
		...
		aiestimatesObject.addProperty(customFieldInJira, newAiestimatesAsString);
	...
	public String getFieldId(String fieldName) {
	 ...
	 public String createField(String fieldName) {
		 ...
		 RestTemplate restTemplate = new RestTemplate();
			...
			String jiraTasksEndpoint = customField_creation_url + "/field" ;
			 String fieldJson = String.format("{\"name\":\"%s\", \"description\":\"%s\", \"type\":\"%s\"}",
					 fieldName, "A custom field for Ai Estimate", "com.atlassian.jira.plugin.system.customfieldtypes:textfield");
           ResponseEntity<String> createResponseEntity = restTemplate.exchange(
           		jiraTasksEndpoint, HttpMethod.POST, new HttpEntity<>(fieldJson, headers), String.class);
...
//	            JsonArray jsonArray = jsonParser.parse(jsonResponse).getAsJsonArray();
	 ...
	 private void assignCustomFieldToScreens(String customFieldId, List<String> screenIds) {
	        ...
	        for (String screenId : screenIds) {
	            ...
	            if (!responseEntity.getStatusCode().is2xxSuccessful()) {
	                System.err.println("Failed to assign custom field to screen with ID " + screenId +
	 ...
	 public void updateLabelsToJira(String taskId, List<String> newLabels) {
	        try {
	            ...
	            if (responseEntity.getStatusCode().is2xxSuccessful()) {
	                ...
	                System.err.println("Failed to update labels for issue ID " + taskId + ". Response: " + responseEntity.getBody());
	            }
	        } catch (Exception e) {
	            // Log the exception
	            e.printStackTrace();
